...
public class ChangeCategoryCommand implements Command {
    ...
    public CommandResult execute(RequestContent requestContent) throws ServletException {
        String name = requestContent.getRequestParameter(ParameterConstant.CATEGORY_NAME);
        ...
        String result;
        try {
            result = categoryService.updateName(categoryId, name);
            if (result.equals(MessageConstant.SUCCESSFULLY_CHANGE_RECORD)) {
        ...
        CommandResult commandResult = CommandResult.setForwardPage(PageConstant.CATEGORY);
